...
Envíe correos electrónicos automatizados posteriores a la compra para WooCommerce: solicitudes de agradecimiento y revisión, una cantidad determinada de días después de realizar un pedido.

== Description ==

El seguimiento envía correos electrónicos automatizados posteriores a la compra a sus clientes de WooCommerce, una cantidad configurable de días después de que un pedido alcanza un estado como Completado.
...
Para cada tipo, estableces si está habilitado, qué estado del pedido lo activa, cuántos días esperar y el asunto y el cuerpo. Los sujetos y cuerpos admiten `{customer}` (nombre), `{order}` (número de pedido) y `{site}` (nombre del sitio).
...
Los desarrolladores pueden ampliar la secuencia a través del filtro "seguimiento/pasos_de secuencia". Cada paso personalizado puede proporcionar su propio estado de activación, retraso, tema y cuerpo mientras se reutiliza el programador idempotente de Followup.

El complemento aún no está en el directorio de WordPress.org. Código fuente y rastreador de problemas en vivo en https://github.com/wppoland/plogins-followup.

== Installation ==

1. Cargue el complemento en `/wp-content/plugins/followup`, o instálelo a través de Complementos -> Añadir nuevo.
2. Actívalo. WooCommerce debe estar activo.
3. Vaya a WooCommerce -> Seguimientos para habilitar los tipos de correo electrónico y editar las plantillas.
...
Usted elige el estado de activación por tipo de correo electrónico (por ejemplo, procesado o completado) y la demora en días antes de que se envíe.
...
1. La pantalla de configuración de Seguimientos: activa cada tipo de correo electrónico y edite su estado de activación, retraso y plantillas.

== External Services ==

El seguimiento no se conecta a ningún servicio externo. No tiene claves API, no envía datos fuera del sitio y no carga nada desde una URL o CDN remota. Todo se ejecuta en su propia instalación de WordPress: las configuraciones se almacenan en las opciones `followup_settings` y `followup_db_version`, y cada seguimiento enviado se registra como meta de pedido `_followup_sent_{type}`, por lo que nunca se envía dos veces. Los correos electrónicos se envían a través del `wp_mail()` de tu sitio utilizando el remitente de su tienda WooCommerce, por lo que viajan mediante cualquier configuración de correo que ya tenga.
...
= 0.1.5 =
* Renombrado a Plogins Followup para WooCommerce para obtener un nombre de complemento más distintivo.
...
* Documente el marcador de posición `{coupon}` para los bloques de cupones de Followup Pro.
